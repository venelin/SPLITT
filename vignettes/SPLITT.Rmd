...
* The [Writing a traversal specification](./SPLITTTraversalSpecification.html) guide shows how to define the application specific data-type and node-traversal operations for a simple mathematical model of trait evolution called "PMM". You will not need to understand the biological interpretation of the model. Rahter, this tutorial will teach you what are the data types and methods that need to be defined in a `TraversalSpecification`-class. I think that this is the ideal place to start. 
* The [View from above](./SPLITTOverview.html) guide overviews the classes in the SPLITT API using a UML class diagram. If you are familiar with UML class diagrams, and/or prefer a top-down approach to understanding a given software, you can read this guide first, then, read the [Writing a traversal specification](./SPLITTTraversalSpecification.html).
* The [Running a traversal](./SPLITTRunTraversal.html) guide shows how to create and run a SPLITT traversal task. This tutorial uses `TraversalSpecification`-class defined in the [Writing a traversal specification](./SPLITTTraversalSpecification.html) to build a simple console appication reading from the standard input `std::cin` and writing to the stadard output `std::cout`. 
* The [Calling SPLITT from an R-package](./SPLITTRcppModules.html) guide shows how to use Rcpp to wrap SPLITT traversal functionality into an R-package. This tutorial uses the `TraversalSpecification`-class defined in the [Writing a traversal specification](./SPLITTTraversalSpecification.html) to build an Rcpp-module. 

Examples of using the SPLITT library can be found in the following locations:

* [PMMUsingSPLITT-console-app](https://github.com/venelin/SPLITT/tree/master/PMMUsingSPLITT-console-app) is the console application described in [Running a traversal](./SPLITTRunTraversal.html). 
* [PMMUsingSPLITT](https://github.com/venelin/PMMUsingSPLITT) is the R-package described in [Calling SPLITT from an R-package](./SPLITTRcppModules.html).
